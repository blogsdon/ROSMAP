...
methodMeans <- aggregate(tdrVec ~ method, data = b, FUN = mean)
methodMeans <- arrange(methodMeans, tdrVec)
sparsityMethodMeans <- aggregate(tdrVec ~ sparsityMethod, data = b, FUN = mean)
...
sparsityMethodMeans2 <- arrange(sparsityMethodMeans2, powerVec)
...
ggplot(b) + theme_bw() + geom_bar(aes(x = method2, y = tdrVec, fill = sparsityMethod2), position = "dodge", stat = "identity") + xlab("Method") + ylab("True Discovery Rate") + scale_fill_discrete(name = "Sparsity Method") + ggtitle("True Discovery Rate")


 ggplot(b) + theme_bw() + geom_bar(aes(x = method3, y = powerVec, fill = sparsityMethod3), position = "dodge", stat = "identity") + xlab("Method") + ylab("Power") + scale_fill_discrete(name = "Sparsity Method") + ggtitle("Power")
