...
echo "https://github.com/blogsdon/ROSMAP/blob/master/runROSMAPbraak12.sh" >> codeBraak12.txt
echo "https://github.com/blogsdon/ROSMAP/blob/master/getROSMAPrnaseqBraak.R" >> codeBraak12.txt

cd /shared/metanetworkSynapse/
qsub -v dataFile="/shared/ROSMAP/BRAAK12rna.csv",pathv="/shared/metanetworkSynapse/",sparrowZ=1,sparrow2Z=1,lassoCV1se=1,lassoCVmin=1,lassoAIC=1,lassoBIC=1,ridgeCV1se=1,ridgeCVmin=1,ridgeAIC=1,ridgeBIC=1,genie3=1,tigressRootN=1,elasticNetAIC=1,elasticNetBIC=1,elasticNetCVmin=1,elasticNetCV1se=1,numberCore=150,outputpath="/shared/ROSMAP/BRAAK12/" -pe orte 150 buildNet.sh
